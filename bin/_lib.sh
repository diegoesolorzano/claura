...
claura_detect_host() {
  ...
  # Plugin hook runners may scrub host-specific environment variables. Infer
  # the host from the hook shell's ancestors before falling back to Claude.
  pid=$(claura_find_host_pid "$PPID")
  if [[ -n "$pid" ]]; then
